...
esp_err_t aes67_audio_set_sample_rate(aes67_audio_handle_t handle, uint32_t sample_rate)
{
    ...
    if (handle->config.sample_rate == sample_rate) {
        return ESP_OK;  /* Already at this rate */
    }

    ESP_LOGI(TAG, "Switching I2S sample rate: %lu -> %lu Hz",
             (unsigned long)handle->config.sample_rate,
             (unsigned long)sample_rate);

    /* Reconfigure clock on both TX and RX channels.
     * i2s_channel_reconfig_std_clock requires the channel to be
     * in the READY state (disabled). */
    i2s_channel_disable(handle->tx_chan);
    i2s_channel_disable(handle->rx_chan);
    ...
    esp_err_t ret = i2s_channel_reconfig_std_clock(handle->tx_chan, &clk_cfg);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "TX clock reconfig failed: %s", esp_err_to_name(ret));
    }

    ret = i2s_channel_reconfig_std_clock(handle->rx_chan, &clk_cfg);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "RX clock reconfig failed: %s", esp_err_to_name(ret));
    }

    i2s_channel_enable(handle->tx_chan);
    i2s_channel_enable(handle->rx_chan);

    handle->config.sample_rate = sample_rate;

    ESP_LOGI(TAG, "I2S sample rate set to %lu Hz (MCLK=%lu Hz)",
             (unsigned long)sample_rate,
             (unsigned long)(sample_rate * 384));
