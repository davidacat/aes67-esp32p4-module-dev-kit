...
esp_err_t aes67_audio_set_sample_rate(aes67_audio_handle_t handle, uint32_t sample_rate)
{
    ...
    ESP_LOGI(TAG, "Switching I2S: %lu -> %lu Hz",
    ...
    i2s_channel_reconfig_std_clock(handle->tx_chan, &clk_cfg);
    i2s_channel_reconfig_std_clock(handle->rx_chan, &clk_cfg);

    i2s_channel_enable(handle->tx_chan);
    i2s_channel_enable(handle->rx_chan);

    handle->config.sample_rate = sample_rate;
    ESP_LOGI(TAG, "I2S now at %lu Hz (MCLK=%lu Hz)",
             (unsigned long)sample_rate, (unsigned long)(sample_rate * 384));
    return ESP_OK;
}
